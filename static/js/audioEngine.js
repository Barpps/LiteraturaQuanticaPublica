...
export class SessionAudio {
  ...
  async start() {
    ...
    const oscToStart = [drone174, tone432, tone528, tone639, tone852, tone963, isoCarrier, lfo, noise, lfoOffset];
    if (toneFund) oscToStart.push(toneFund);
    if (binL) { oscToStart.push(binL); }
    if (binR) { oscToStart.push(binR); }
    oscToStart.forEach(n => n.start(now));
